...
export default function BooksFormPage() {
    ...
    async function saveBook(ev) {
        ev.preventDefault();
        const bookData =  {
            title, author, isbn, numberOfPages, address, addedPhotos,
    ...
    return (
        <div className=" flex-c ml-20 p-2 mt-4 bg-black rounded-3xl">
            <AccountNav/>
            <div className=" flex-c bg-lightblack text-1xl p-2 items-center rounded-3xl text-center"> 
                <div className=" text-primary bg-black p-5 py-3">
                    ...
                    {preInput('Author', 'Author of this book, Writer of this book')}
                        <input type="text" value={author} onChange={ev => setAuthor(ev.target.value)} placeholder="author, for example: J.K Rowling"/>    
                    {preInput('ISBN-10', 'Unique numeric book identifier')}
                    ...
                    {preInput('Address', 'Address to this publishment')}
                        <input type="text" value={address} onChange={ev => setAddress(ev.target.value)} placeholder="Landmark"/>    

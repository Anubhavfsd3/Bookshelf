...
export default function PhotosUploader({addedPhotos,onChange}) {
    ...
    function uploadPhoto(ev) {
        ...
        axios.post('/upload', data, {
            ...
            console.log("Hello",filenames)
            const path = filenames[0].replace(/\\/g, '/');
            setMyPhotos(filenames)
            onChange(prev => {
    ...
    return(
        <>
            ...
            <div className="flex-c mt-7 grid gap-2 grid-cols-3 md:grid-cols-4 lg:grid-cols-6 ml-9">
                {/* {console.log(Array.isArray(myPhotos))}
                {myPhotos?.map((photo, idx) => (
                    // <img src={photo} key={idx} alt="preview" />
                    <img src={`/${filePath}`} alt="Uploaded" style={{ width: '300px' }} />
                ))} */}
                {myPhotos && (
        <div>
          <h3>Uploaded Image:</h3>
          <img src={`/${myPhotos}`} alt="Uploaded" style={{ width: '300px' }} />
        </div>
      )}
                {/* {myPhotos > 0 && myPhotos?.map((link) => (
                    <div className="h-32 flex relative" key={link}>
                        {console.log('I am inside UI', link)}
                        <img className="rounded-2xl w-full object-cover bg-capitalize" src={`http://localhost:3002/uploads/${link}`} alt="Preview"/>

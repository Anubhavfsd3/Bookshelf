...
app.post('/Books', (req,res) => {
    const {token} = req.cookies;
    const {
        title,author,isbn,numberOfPages,address,addedPhotos,description,price,
    ...
    jwt.verify(token, jwtSecret, {}, async (err, userData) => {
        ...
        const bookDoc = await Book.create({
            ...
            title,author,isbn,numberOfPages,address,photos:addedPhotos,description,
            perks,extraInfo,secureCheckIn,secureCheckout,
        });
        res.json(bookDoc);
    });
...
app.put('/Bookks', async (req,res) => {
    const {token} = req.cookies;
    const {
        id,title,author,isbn,numberOfPages,address,addedPhotos,description,
    ...
    jwt.verify(token, jwtSecret, {}, async(err, userData) => {
        ...
        if (userData.id === bookDoc.owner.toString()) {
            bookDoc.set({
                id,title,author,isbn,numberOfPages,address,photos:addedPhotos,description,
                perks,extraInfo,secureCheckIn,secureCheckout,price,
            });
            await bookDoc.save();
            res.json('ok');
...
app.post('/bookings', async (req, res) => {
    ...
    const {
      book,checkIn,checkOut,numberOfBooks,name,email,price,
    } = req.body;
    Booking.create({
      book,checkIn,checkOut,numberOfBooks,name,email,price,
      user:userData.id,
